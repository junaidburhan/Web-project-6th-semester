...
<div class="col-md-12">
    <a href="#insertmodal" data-toggle="modal" class="btn btn-sm active" style="font-weight:bold !important">اضافه نمودن کارمند جدید</a>
  </div>
<?php } ?>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title text-center"  style="font-weight:bold;"> فورم ثبت نام کارمندان </h4>
      </div>
      ...
      <div class="modal-footer">
          ...
          <button type="button" class="btn btn-success btn-sm" onclick="InsertEmployee()">ثبت کردن</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal" id="editModal" role="dialog">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title text-center"  style="font-weight:bold;">فورم  ویرایش  کارمندان</h4>
      </div>
      <div class="modal-body" style="padding:0px 20px !important">
      ...
      <div class="modal-footer">
          ...
          <button type="button" class="btn btn-success btn-sm" onclick="editEmployee()" data-dismiss="modal">ویرایش کردن</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
  ...
  function ShowEmployees(){
     ...
     let xhr = new XMLHttpRequest();
        xhr.open("POST","php/select-employees.php",true);
        xhr.onload = ()=>{
  ...
  function InsertEmployee(){
      const form = document.querySelector(".formInsert");
        let xhr = new XMLHttpRequest();
        xhr.open("POST","php/insert-employee.php",true);
        xhr.onload = ()=>{
            if(xhr.readyState === XMLHttpRequest.DONE)
            {
                if(xhr.status == 200)
  ...
  function editEmployee(){
    const form = document.querySelector(".formEdit");
      let xhr = new XMLHttpRequest();
        xhr.open("POST","php/edit-employee.php",true);
        xhr.onload = ()=>{
            if(xhr.readyState === XMLHttpRequest.DONE)
            {
                if(xhr.status == 200)
                {
                  let data = xhr.response;
                  if(data =="تمام معلومات ضروری میباشد" || data == "این ایمیل آدرس در سیستم وجود دارد" || data=="خطا در سیستم" || data=="فارمت عکس ( jpg , jpeg , png )  باشد" || data =="ایمیل آدرس شما درست نمیباشد")
                  {
  ...
  // end of hide and show function
   ...
   function MoreInfo(ID){
      ...
      const d = document.querySelector("#detailsDiv");
        let xhr = new XMLHttpRequest();
          xhr.open("POST","php/details-employee.php",true);
          xhr.onload = ()=>{
              if(xhr.readyState === XMLHttpRequest.DONE)
              {
                  if(xhr.status == 200)
...
btn.onkeyup = ()=>{
    ...
    let divcontent = document.querySelector("#ContentDiv");
      let xhr = new XMLHttpRequest();
        xhr.open("POST","php/search-employee.php",true);
        xhr.onload = ()=>{
            if(xhr.readyState === XMLHttpRequest.DONE)
            {
                if(xhr.status == 200)
