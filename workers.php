...
<div class="col-md-12">
    <a href="#insertmodal" data-toggle="modal" class="btn btn-sm active" style="font-weight:bold !important">اضافه نمودن کارگر جدید</a>
</div>
<?php } ?>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title text-center"  style="font-weight:bold;">فورم  ثبت  نام کارگران</h4>
      </div>
      <div class="modal-body" style="padding:0px 20px !important">
          <form method="POST" enctype="multipart/form-data" class="formInsert">
            ...
            <div class="form-group col-md-3">
                ...
                <input type="email" name="email" value="" class="form-control" placeholder="ایمیل آدرس">
            </div>
            <div class="form-group col-md-3">
      ...
      <div class="modal-footer">
          ...
          <button type="button" class="btn btn-success btn-sm" onclick="InsertWorker()">ثبت کردن</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal" id="editModal" role="dialog">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title text-center"  style="font-weight:bold;">فورم  ویرایش  کارگران</h4>
      </div>
      <div class="modal-body" style="padding:0px 20px !important">
      ...
      <div class="modal-footer">
          ...
          <button type="button" class="btn btn-success btn-sm" onclick="editWorker()" data-dismiss="modal">ویرایش کردن</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal" id="detailsModal" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      		<h4  class="text-center"><b>جزیٔیات در باره کارمند</b></h4>
      </div>
...
<script type="text/javascript">
  ...
  function ShowWorkers()
  {
     ...
     let xhr = new XMLHttpRequest();
        xhr.open("POST","php/select-workers.php",true);
        xhr.onload = ()=>
		{
  ...
  function InsertWorker(){
      const form = document.querySelector(".formInsert");
        let xhr = new XMLHttpRequest();
        xhr.open("POST","php/insert-worker.php",true);
        xhr.onload = ()=>{
            if(xhr.readyState === XMLHttpRequest.DONE)
            {
                if(xhr.status == 200)
                {
                  let data = xhr.response;
                  if(data == "تمام معلومات ضروری میباشد" || data=="خطا در سیستم" || data =="فارمت عکس ( jpg , jpeg , png ) باشد" || data=="ایمیل آدرس شما درست نمیباشد" || data=="این ایمیل آدرس در سیستم وجود دارد")
                  {
  ...
  function SetValues(ID,NAME,LNAME,FNAME,ENAME,ELNAME,GENDER,EMAIL,PHONE,STATUS,BIRTH,HIRE,PRO,POS,EPOS,FAC,JOB,DEG,PHOTO){
    let id = ID;
    let name = NAME;
    let lname = LNAME;
    let fname = FNAME;
    ...
    const elname1 = document.querySelector("#elname");
    ...
    const birth_date1 = document.querySelector("#birth_date");
    const hire_date1 = document.querySelector("#hire_date");
    const province1 = document.querySelector("#province");
    const position1 = document.querySelector("#position");
    const eposition1 = document.querySelector("#eposition");
    const faculty1 = document.querySelector("#faculty");
    const job1 = document.querySelector("#job");
    const degree1 = document.querySelector("#degree");
    ...
    fname1.value = fname;
    ename1.value = ename;
    elname1.value = elname;
    gender1.value = gender;
    email1.value = email;
    phone1.value = phone;
    status1.value = status;
    birth_date1.value = birth;
    hire_date1.value = hire;
    province1.value = province;
    position1.value = position;
    eposition1.value = eposition;
    faculty1.value = faculty;
    job1.value = job;
    degree1.value = degree;
    img.src = photo;
 }
 ...
 function editWorker(){
    const form = document.querySelector(".formEdit");
      let xhr = new XMLHttpRequest();
        xhr.open("POST","php/edit-worker.php",true);
        xhr.onload = ()=>{
            if(xhr.readyState === XMLHttpRequest.DONE)
            {
                if(xhr.status == 200)
                {
                  let data = xhr.response;
                  if(data =="تمام معلومات ضروری میباشد" || data =="ایمیل آدرس شما درست نمیباشد" || data == "این ایمیل آدرس در سیستم وجود دارد" || data=="خطا در سیستم" || data=="فارمت عکس ( jpg , jpeg , png ) باشد")
                  {
  ...
  // end of hide and show function
   ...
   function MoreInfo(ID){
      ...
      const d = document.querySelector("#detailsDiv");
        let xhr = new XMLHttpRequest();
          xhr.open("POST","php/details-worker.php",true);
          xhr.onload = ()=>{
              if(xhr.readyState === XMLHttpRequest.DONE)
              {
                  if(xhr.status == 200)
...
btn.onkeyup = ()=>{
    ...
    let divcontent = document.querySelector("#ContentDiv");
      let xhr = new XMLHttpRequest();
        xhr.open("POST","php/search-worker.php",true);
        xhr.onload = ()=>{
            if(xhr.readyState === XMLHttpRequest.DONE)
            {
                if(xhr.status == 200)
