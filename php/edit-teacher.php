      ...
      else
      {
        if($_FILES['photo']['name'] != "")
        {
          ...
          if(in_array($img_ext,$extensions))
          {
                ...
                {
                  $query = mysqli_query($conn,"UPDATE employees SET pname ='$name',plname='$lname',fname='$fname',name='$ename',lname='$elname',gender='$gender',email='$email',phone='$phone',status='$status',birth_date='$birth_date',hire_date='$hire_date', province_id = $province, rotba_id = $rotba, positions = '$position', en_positions = '$eposition', faculty_id = $faculty, job_id = $job , degree_id = $degree ,photo ='$path' WHERE id = $id");
                    if($query)
                    {
         ...
         {
           $query = mysqli_query($conn,"UPDATE employees SET pname ='$name',plname='$lname',fname='$fname',name='$ename',lname='$elname',gender='$gender',email='$email',phone='$phone',status='$status',birth_date='$birth_date',hire_date='$hire_date', province_id = $province, rotba_id = $rotba, positions = '$position', en_positions = '$eposition', faculty_id = $faculty, job_id = $job , degree_id = $degree WHERE id = $id ");
           if($query)
               $output .="موفقانه ویرایش گردید";
           else
               $output .= "خطا در سیستم";
