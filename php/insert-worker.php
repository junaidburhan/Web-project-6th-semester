<?php
 ...
 if($name !="" AND $ename !="" AND $lname !="" AND $elname !="" AND $fname !="" AND $phone !="" AND $hire_date !="" AND $birth_date !="" AND $position !="" AND $eposition !="" AND $_FILES['photo']['name'] !="")
 {
	if($email != ''){
			...
			$q = mysqli_query($conn,"SELECT name FROM employees WHERE email = '$email' LIMIT 1");
				...
				{		
					...
					if(in_array($img_ext,$extensions))
					{
						...
						{
							$query = mysqli_query($conn,"INSERT INTO employees VALUES (null,'$name','$lname','$fname','$ename','$elname','$gender','$email','$phone','$status','$birth_date','$hire_date','$position','$eposition','w','$faculty','$job','$degree','$province',null,'$path')");
							if($query)
							{
	...
	else
	{
          ...
          if(in_array($img_ext,$extensions))
          {
                ...
                if(move_uploaded_file($_FILES['photo']['tmp_name'],$img))
                {
                  $query = mysqli_query($conn,"INSERT INTO employees VALUES (null,'$name','$lname','$fname','$ename','$elname','$gender',null,'$phone','$status','$birth_date','$hire_date','$position','$eposition','w','$faculty','$job','$degree','$province',null,'$path')");
                    if($query)
                    {
                      $output .="موفقانه دخیره گردید";
                    }
