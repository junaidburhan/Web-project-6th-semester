<?php
 ...
 if($name !="" AND $ename !="" AND $lname !="" AND $elname !="" AND $fname !="" AND $phone !="" AND $email !="" AND $hire_date !="" AND $birth_date !="" AND $position !="" AND $eposition !="")
 {
      ...
      else
      {
        if($_FILES['photo']['name'] !="")
        {
          ...
          if(in_array($img_ext,$extensions))
          {
                ...
                $img = "uploads/employees/".time().rand(1,99999).basename($_FILES['photo']['name']);
                $path = "php/".$img;
                if(move_uploaded_file($_FILES['photo']['tmp_name'],$img))
                {
                  $query = mysqli_query($conn,"UPDATE employees SET pname ='$name',plname='$lname',fname='$fname',name='$ename',lname='$elname',gender='$gender',email='$email',phone='$phone',status='$status',birth_date='$birth_date',hire_date='$hire_date', positions = '$position', en_positions = '$eposition',province_id = $province,degree_id = $degree, faculty_id = $faculty, job_id = $job ,photo ='$path' WHERE id = $id ");
                    if($query)
                    {
         ...
         {
           $query = mysqli_query($conn,"UPDATE employees SET pname ='$name',plname='$lname',fname='$fname',name='$ename',lname='$elname',gender='$gender',email='$email',phone='$phone',status='$status',birth_date='$birth_date',hire_date='$hire_date', positions = '$position', en_positions = '$eposition',province_id = $province,degree_id = $degree, faculty_id = $faculty, job_id = $job WHERE id = $id ");
           if($query)
               $output .="موفقانه ویرایش گردید";
           else
               $output .= "خطا در سیستم";
